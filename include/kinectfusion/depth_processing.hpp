...
template <MemorySpace Space = MemorySpace::Host> struct VertexNormalMapsView {
...
[[nodiscard]] inline ConstHostVertexNormalMapsView
view(const VertexNormalMaps &maps) {
...
struct DepthProcessingLevel {
  DepthProcessingLevel(image_proc::DepthImage depth,
                       ...
                       image_proc::Vector3fImage vertices,
                       image_proc::Vector3fImage normals)
      : depth_image(std::move(depth)), intrinsics(camera_intrinsics),
...
compute_normals_central_differences(const image_proc::Vector3fImage &vertices,
...
[[nodiscard]] image_proc::DepthImage
bilateral_filter_depth(const image_proc::DepthImage &depth_image,
                       const DepthProcessingOptions &options = {});
...
[[nodiscard]] image_proc::DepthImage
build_depth_pyramid_level(const image_proc::DepthImage &depth_image,
                          const DepthProcessingOptions &options = {});
...
[[nodiscard]] image_proc::Vector3fImage
project_depth_to_vertices(const image_proc::DepthImage &depth_image,
