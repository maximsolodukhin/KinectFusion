...
constexpr std::uint16_t grid_x = 32U;
constexpr std::uint16_t grid_y = 8U;

__host__ __device__ __forceinline__ float depth_to_meters(std::uint16_t depth,
                                           float depth_scale) {
...
[[nodiscard]] inline bool usable_depth(std::uint16_t raw,
    const DepthProcessingOptions& options, float& depth_meters) {
  if (raw == 0) {
    return false;
  }
...
__device__ bool downsample_depth_block(
    const image_proc::DeviceImageView<std::uint16_t> depth_image,
  ...
  out = static_cast<std::uint16_t>((sum + (count / 2U)) / count);
...
image_proc::DepthImage bilateral_filter_depth(
  ...
  for (int row = 0; row < height; ++row) {
    for (int col = 0; col < width; ++col) {
      const auto center = depth_image.at(
          static_cast<unsigned int>(col), static_cast<unsigned int>(row));
...
__global__ void build_depth_pyramid_level(
    const image_proc::DeviceImageView<std::uint16_t> depth_image,
    DepthProcessingOptions options,
    image_proc::DeviceImageView<std::uint16_t> out) {
  ...
  if (downsample_depth_block(depth_image, x, y, options, value)) {
    out.at(x, y) = value;
  }
}
...
std::vector<DepthProcessingLevel> build_surface_pyramid(
  ...
  image_proc::DepthImage current =
      kinectfusion_cuda::bilateral_filter_depth(depth_image, options);
  for (unsigned int level = 0; level < options.levels; ++level) {
    ...
    image_proc::Vector3fImage vertices =
        kinectfusion_cuda::project_depth_to_vertices(
            current, level_intrinsics, camera_pose, options);
    image_proc::Vector3fImage normals =
        kinectfusion_cuda::compute_normals_central_differences(
            vertices, options);
    if (level + 1U < options.levels) {
      pyramid.emplace_back(current, level_intrinsics, std::move(vertices), std::move(normals));

      image_proc::DepthImage next{
          current.width() / downsample_factor, current.height() / downsample_factor};
      std::uint16_t* in = nullptr;
      std::uint16_t* out = nullptr;
      cudaMalloc(&in, current.width() * current.height() * sizeof(std::uint16_t));
      cudaMalloc(&out, next.width() * next.height() * sizeof(std::uint16_t));
      cudaMemcpy(in, current.data().data(), current.width() * current.height() * sizeof(std::uint16_t), cudaMemcpyHostToDevice);
      cudaMemset(out, 0, next.width() * next.height() * sizeof(std::uint16_t));
      const image_proc::DeviceImageView<std::uint16_t> depth_image_view{.data=in, .width=current.width(), .height=current.height()};
      const image_proc::DeviceImageView<std::uint16_t> out_view{.data=out, .width=next.width(), .height=next.height()};
      const dim3 block(grid_x, grid_y);
      const dim3 grid((next.width() + block.x - 1) / block.x, (next.height() + block.y - 1) / block.y);
      kinectfusion_cuda::build_depth_pyramid_level<<<grid, block>>>(depth_image_view, options, out_view);
      cudaDeviceSynchronize();
      cudaMemcpy(next.data().data(), out, next.width() * next.height() * sizeof(std::uint16_t), cudaMemcpyDeviceToHost);
      cudaFree(in);
      cudaFree(out);
      current = std::move(next);
    } else {
      pyramid.emplace_back(std::move(current),
          level_intrinsics, std::move(vertices), std::move(normals));
    }
  }
  return pyramid;
}
