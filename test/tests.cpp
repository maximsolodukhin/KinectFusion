...
[[nodiscard]] SyntheticSurface make_identical_surface(
  ...
  SyntheticSurface surface{
      .live = {.vertices =
                   kinectfusion::image_proc::Image<Eigen::Vector3f>{width,
                                                                    height},
               .normals =
                   kinectfusion::image_proc::Image<Eigen::Vector3f>{width,
                                                                    height}},
  ...
  for (unsigned int y = 0; y < height; ++y) {
    for (unsigned int x = 0; x < width; ++x) {
      ...
      const auto point = Eigen::Vector3f{
...
TEST_CASE("Depth processing back-projects a flat depth image",
  ...
  const auto vertices = kinectfusion::back_project_depth(depth, intrinsics);
  ...
  const auto normals = kinectfusion::compute_normals(
      vertices, kinectfusion::DepthProcessingOptions{
                    .normal_computation =
...
TEST_CASE("Depth pyramid rejects mixed-depth neighborhoods",
  ...
  const auto pyramid = kinectfusion::build_depth_pyramid(
      depth, kinectfusion::DepthProcessingOptions{
                 .levels = 2,
                 .max_normal_depth_jump = 0.1F});

  REQUIRE(pyramid.size() == 2);
  REQUIRE(pyramid.at(1).width() == 1);
  REQUIRE(pyramid.at(1).height() == 1);
  REQUIRE(pyramid.at(1).at(0, 0) == 0);
}
...
TEST_CASE("Volume integrates and raycasts a synthetic depth plane", "[volume]") {
  ...
  const auto maps = volume.raycast(kinectfusion::RaycastOptions{
      .intrinsics = intrinsics,
      .width = width,
      .height = height,
      .camera_to_world = Eigen::Matrix4f::Identity(),
      .min_depth = 0.2F,
      .max_depth = 2.0F,
      .step_scale = 1.0F,
...
TEST_CASE("Projective ICP converges on identical synthetic maps",
  ...
  const auto result = tracker.estimate_pose(
      surface.live, surface.model, intrinsics, Eigen::Matrix4f::Identity(),
      Eigen::Matrix4f::Identity());

  REQUIRE(result.status == kinectfusion::ProjectiveIcpStatus::converged);
  REQUIRE(result.converged);
  REQUIRE(result.correspondences == 9);
  REQUIRE(result.update_translation == Catch::Approx(0.0F).margin(1.0e-5F));
  REQUIRE(result.update_rotation == Catch::Approx(0.0F).margin(1.0e-5F));
}
...
TEST_CASE("Projective ICP recovers a small pose perturbation",
  ...
  const kinectfusion::ProjectiveIcpTracker tracker{
      kinectfusion::ProjectiveIcpOptions{
          ...
          .max_condition_number = 1.0e12F}};
  const auto result =
      tracker.estimate_pose(surface.live, surface.model, intrinsics,
  ...
  REQUIRE(result.mean_point_distance == Catch::Approx(0.0F).margin(1.0e-4F));
}
...
TEST_CASE("Projective ICP rejects empty correspondence sets",
  ...
  kinectfusion::VertexNormalMaps live_maps{
      .vertices = kinectfusion::image_proc::Image<Eigen::Vector3f>{width, height},
      .normals = kinectfusion::image_proc::Image<Eigen::Vector3f>{width, height}};
  kinectfusion::SurfaceMaps model_maps{
      .points = kinectfusion::image_proc::Image<Eigen::Vector3f>{width, height},
      .normals = kinectfusion::image_proc::Image<Eigen::Vector3f>{width, height},
      .colors = kinectfusion::image_proc::ColorImage{width, height}};
  ...
  const auto result = tracker.estimate_pose(
      live_maps, model_maps, kinectfusion::CameraIntrinsics{},
      Eigen::Matrix4f::Identity(), Eigen::Matrix4f::Identity());

  REQUIRE_FALSE(result.converged);
  REQUIRE(result.status ==
          kinectfusion::ProjectiveIcpStatus::too_few_correspondences);
  REQUIRE(result.correspondences == 0);
}
